<template>
    <page_layouts :pageTitle="pageTitle" :showAddButton="showAddButton" :headers="headers">
        <template v-slot:page_top>
            <page-top :treger="openModal"  page-title="Days  -- "></page-top>
        </template>
        <template v-slot:data>
            <tr v-for="(data , index) in dataList.data">
                ...
                <td>{{data.name}}</td>
                <td>
                    <a v-if="('day_status')" @click="changeStatus(data.id)" v-html="showStatus(data.status)"></a>
                </td>
                <td>
                    <div class="d-flex gap-3 ">
                        <p v-if="can('day_edit','day_update')" @click="editData(data,data.id)"><i class="fas fa-edit fs-6"></i></p>
                        <p v-if="can('day_destroy')" @click="deletData(data.id)"><i class="fas fa-trash text-danger fs-6"></i></p>
                    </div>
                </td>
            </tr>
        </template>
    ...
    <modal-layouts modalTitle="Add Class">
        <div class="form-group">
            <label class="col-form-label">Day Name</label>
